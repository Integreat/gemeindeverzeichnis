...
class DetailWWW implements ImportInterface, LoggerAwareInterface
{
    /**
    ...
    public function import()
    {
        ...
        $this->logger->info("Updating details from WWW.");
        /**
        ...
        while ($row = $res->fetch_assoc()) {
            ...
            $this->logger->info("Update with search " . $row['key']);
            ...
            $stmt->bind_param("sssssssssssss", $data['county'], $data['state'], $data['district'], $data['type'],
                $data['population'], $data['population_male'], $data['population_female'], $data['area'],
                $data['address_recipient'], $data['address_street'], $address_zip, $address_city, $row['key']);
            if ($stmt->execute()) {
                $this->logger->info("Updated " . $row['key']);
            }
            $stmt->close();
        }
    }
